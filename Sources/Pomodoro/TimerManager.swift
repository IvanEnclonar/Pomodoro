...
class TimerManager: ObservableObject {
    ...
    var validatedCompletionSound: String { Self.availableSounds.contains(completionSound) ? completionSound : "Ping" }
    ...
    private func tick() {
        ...
        if timeRemaining > 0 {
            ...
            // Play ding sound
            NSSound(named: NSSound.Name(validatedCompletionSound))?.play()
            // Show the window
