...
class TimerManager: ObservableObject {
    ...
    // Security: Secure accessors for AppStorage values to prevent tampering or invalid values
    private var safeFocusDuration: Int {
        return max(1, min(1440, focusDurationMinutes))
    }

    private var safeShortBreakDuration: Int {
        return max(1, min(240, shortBreakDurationMinutes))
    }

    private var safeLongBreakDuration: Int {
        return max(1, min(240, longBreakDurationMinutes))
    }

    private var safeStreakMinMinutes: Int {
        return max(1, min(1440, streakMinMinutes))
    }
    ...
    private func totalDuration(for state: SessionState) -> Int {
        switch state {
        case .focus: return safeFocusDuration * 60
        case .shortBreak: return safeShortBreakDuration * 60
        case .longBreak: return safeLongBreakDuration * 60
        }
    }
